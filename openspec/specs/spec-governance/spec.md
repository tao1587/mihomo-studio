...
公开仓库 SHALL 使用最小权限 GitHub Actions：提交与 pull request 运行规范、前端和 Rust 验证；只有 `v*` 标签工作流可获得 `contents: write` 并创建 Release。引用的 Actions SHALL 固定到完整 commit SHA。
...
- **GIVEN** 维护者推送 `v*` 标签
- **WHEN** Release 工作流运行
- **THEN** macOS Apple Silicon、macOS Intel、Linux 和 Windows 安装包分别构建
- **AND** 产物发布到该标签对应的 GitHub Release。
...
| `scripts/check-github-workflows.mjs` | CI/Release 触发器、权限、平台矩阵和 Action 固定版本门禁 |
| `.github/workflows/ci.yml` | push/PR 的 OpenSpec、前端和 Rust 验证 |
| `.github/workflows/release.yml` | `v*` 标签的三平台桌面安装包与 GitHub Release |
